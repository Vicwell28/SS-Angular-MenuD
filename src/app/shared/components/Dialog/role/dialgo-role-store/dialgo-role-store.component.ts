...
@Component({
  selector: 'app-dialgo-role-store',
  templateUrl: './dialgo-role-store.component.html',
  styles: ['.example-full-width {width: 100%;}'],
})
export class DialgoRoleStoreComponent implements OnInit {
  icons: iconI[] = [
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialgoRoleStoreComponent>,
  ...
  ok(): void {
    ...
    this.roleService
      .postStoreRole(this.categoryForm.value)
      .subscribe((datos) => {
        if (datos.length >= 1) {
